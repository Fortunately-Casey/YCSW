<template>
        <div class="content">
            <div class="header">
                ...
                <div class="water">用 水 量（立方千米）</div>
                <div class="operation">操 作</div>
            </div>
            <ul class="table">
                <li v-for="(item,index) in comsuptionList"
                   :key="index"
                   :class="index === 1||index === 3?'gray':''"
                   >
                    <span class="number">{{index + 1}}</span>
                    <span class="name">{{item.CompanyName}}</span>
                    <span class="water">{{item.Capacity}}</span>
...
export default {
  ...
  computed: {
    ...mapGetters(['comsuptionList'])
  },
  methods: {
    ...mapActions({
      setDetail: 'setComsuptionDetail',
