  ...
  return declare([InfoWindowBase, Evented],
    {
      constructor: function (parameters) {
        ...
        this._arraw = domConstruct.create('div', { 'class': 'arrowbehind' }, this.domNode)
        this._arraw = domConstruct.create('div', { 'class': 'arrow' }, this.domNode)
      ...
      setMap: function (map) {
        ...
        map.on('pan', lang.hitch(this, function (pan) {
          ...
          if (this.isShowing) {
            if (showScreenPoint != null) { this._showInfoWindow(showScreenPoint.x + movePoint.x, showScreenPoint.y + movePoint.y) }
          }
        }))
      ...
      show: function (location) {
        ...
        if (location.spatialReference) { location = this.map.toScreen(location) }
        var left = location.x - infoWidth / 2
        var top = location.y - infoHeight - 45
        showScreenPoint = location
        if (top < 5) { initScreenCenter.y = initScreenCenter.y + top - 5 }
        if (left < 5) { initScreenCenter.x = initScreenCenter.x + left - 5 }
        this._showInfoWindow(showScreenPoint.x, showScreenPoint.y)
